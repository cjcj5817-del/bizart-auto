...
# ── SMTP 발송 ──────────────────────────────────────────────────────
def send_mail(mail_from, mail_id, mail_pw, subject, body, attach_bytes, attach_name):
    msg = MIMEMultipart()
    msg['From']    = mail_from
    ...
    with smtplib.SMTP(SMTP_SERVER, SMTP_PORT, timeout=15) as server:
        ...
        server.login(mail_id, mail_pw)   # 로그인은 아이디로, From은 이메일로
...
with col_cfg:
    ...
    mail_from_input = st.text_input(
        "📧 보내는 이메일 주소",
        value="",
        placeholder="예: hong@valuemark.co.kr",
        help="발송자로 표시될 이메일 주소"
    )
    col_id1, col_id2 = st.columns(2)
    with col_id1:
        mail_corp_input = st.text_input(
            "🏢 회사코드",
            value="VM-100",
            help="비즈비웍스 로그인 시 사용하는 회사코드"
        )
    with col_id2:
        mail_id_input = st.text_input(
            "👤 아이디",
            value="",
            placeholder="예: VM1802006",
            help="비즈비웍스 로그인 아이디"
        )
    mail_pw_input = st.text_input(
        "🔑 비즈비웍스 비밀번호",
        type="password",
        help="입력 시 처리 완료 후 자동 발송됩니다. 비워두면 메일 초안만 표시됩니다."
    )
...
if not uploaded:
    ...
    if st.button("🚀 자동화 실행", type="primary", use_container_width=True):
        ...
        # 비즈비웍스 SMTP 아이디 형식: 아이디@회사코드 (예: VM1802006@VM-100)
        smtp_id = f"{mail_id_input}@{mail_corp_input}" if mail_id_input and mail_corp_input else mail_id_input
        ...
        # 이메일 + 아이디 + 비밀번호 모두 입력 시 즉시 자동 발송
        if mail_pw_input and mail_from_input and mail_id_input:
            with st.spinner("📨 메일 발송 중..."):
                try:
                    send_mail(mail_from_input, smtp_id, mail_pw_input,
                              stats["mail_subject"], stats["mail_body"],
                              xlsx_out, stats["attach_name"])
                    st.session_state["mail_sent"] = True
                except smtplib.SMTPAuthenticationError:
                    st.session_state["mail_error"] = "로그인 실패 — 아이디/비밀번호를 확인해주세요."
...
if st.session_state.get("processed"):
    ...
    if not mail_sent:
        ...
        with col_a:
            from2 = st.text_input("보내는 이메일 주소", placeholder="hong@valuemark.co.kr", key="from2")
        with col_b:
            id2   = st.text_input("비즈비웍스 로그인 아이디", placeholder="hong 또는 전체 이메일", key="id2")
        with col_c:
            pw2   = st.text_input("비즈비웍스 비밀번호", type="password", key="pw2")
        if st.button("📨 지금 메일 발송", type="primary"):
            if from2 and id2 and pw2:
                with st.spinner("발송 중..."):
                    try:
                        send_mail(from2, id2, pw2, stats["mail_subject"], stats["mail_body"],
                ...
                st.warning("이메일 주소, 아이디, 비밀번호를 모두 입력해주세요.")
