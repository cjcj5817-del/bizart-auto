...
if not uploaded:
    ...
    if st.button("🚀 자동화 실행", type="primary", use_container_width=True):
        ...
        # 이메일 + 비밀번호 입력 시 즉시 자동 발송
        if mail_pw_input and mail_from_input:
            with st.spinner("📨 메일 발송 중..."):
                try:
                    send_mail(mail_from_input, mail_from_input, mail_pw_input,
        ...
        if st.button("📨 지금 메일 발송", type="primary"):
            if from2 and pw2:
                with st.spinner("발송 중..."):
                    try:
                        send_mail(from2, from2, pw2, stats["mail_subject"], stats["mail_body"],
