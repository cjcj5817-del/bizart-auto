...
def process_from_files(ke_bytes, km_bytes, jw_bytes, addr_bytes, existing_bytes, mak):
    ...
    # ── 5. 기존 발송명단 로드 → 중복 확인 (선택) ────────────────
    existing = set()
    if existing_bytes:
        wb_ex = openpyxl.load_workbook(io.BytesIO(existing_bytes), data_only=True)
        ws_ex = wb_ex.active
        for row in ws_ex.iter_rows(min_row=3, values_only=True):
            # 증권번호 = 마지막 열 (10번째)
            v = row[9] if len(row) > 9 else None
            if v: existing.add(sv(v))
        log.append(f"기존 발송명단: {len(existing)}건 (중복 확인)")
    ...
    log.append(f"계약등록 처리 — 전체: {total}건 | 최종대상: {ok}건 | "
                f"계약상태제외: {ex_cont}건 | 금액미달: {ex_amt}건 | "
                f"주소없음: {no_addr_cnt}건 | 인사없음: {na_send}건 | 중복: {dup}건")

    # ── 7. 발주 첨부 xlsx 생성 (이번 달 신규 추가분만) ──────────
    ...
    log.append(f"발주 엑셀 생성 — {attach_name} ({ok}건)")

    stats = dict(
        total=total, ok=ok, ex_cont=ex_cont, ex_amt=ex_amt,
...
if st.session_state.get("processed"):
    ...
    c1.metric("신규 계약 전체",  f"{stats['total']}건")
    c2.metric("발송명단 추가",   f"{stats['ok']}건")
    c3.metric("계약상태 제외",   f"{stats['ex_cont']}건")
    c4.metric("금액 미달 제외",  f"{stats['ex_amt']}건")
    c5.metric("주소 없어 제외",  f"{stats['no_addr']}건")
    c6.metric("중복 제외",       f"{stats['dup']}건")
    ...
    # 처리 로그
    with st.expander("📋 상세 처리 로그", expanded=(stats['ok'] == 0)):
        for line in stats["log"]:
            st.text(line)
    if stats['ok'] == 0:
        st.warning("⚠️ 발송명단에 추가된 건수가 0건입니다. 위 로그에서 제외 원인을 확인해주세요.")
