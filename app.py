...
with col_cfg:
    ...
    mail_from_input = st.text_input(
        ...
        help="비즈비웍스에 로그인할 때 쓰는 이메일 주소를 입력하세요."
    )
...
if not uploaded:
    ...
    if st.button("🚀 자동화 실행", type="primary", use_container_width=True):
        ...
        # 이메일 + 비밀번호 모두 입력 시 즉시 자동 발송
        if mail_pw_input and mail_from_input:
            with st.spinner("📨 메일 발송 중..."):
                try:
                    send_mail(mail_from_input, mail_pw_input, stats["mail_subject"],
                              stats["mail_body"], xlsx_out, stats["attach_name"])
                    st.session_state["mail_sent"] = True
                except smtplib.SMTPAuthenticationError:
                    st.session_state["mail_error"] = "로그인 실패 — 이메일 주소와 비밀번호를 확인해주세요."
...
if st.session_state.get("processed"):
    ...
    if not mail_sent:
        ...
        with col_a:
            from2 = st.text_input("내 이메일 주소", placeholder="hong@valuemark.co.kr", key="from2")
        with col_b:
        ...
        if st.button("📨 지금 메일 발송", type="primary"):
            if from2 and pw2:
                with st.spinner("발송 중..."):
                    try:
                        send_mail(from2, pw2, stats["mail_subject"], stats["mail_body"],
                                  xlsx_out, attach_name)
                        st.success(f"✅ 메일 발송 완료! 수신: {MAIL_TO1}, {MAIL_TO2}")
                    except smtplib.SMTPAuthenticationError:
                        st.error("❌ 로그인 실패 — 이메일 주소와 비밀번호를 확인해주세요.")
                    except Exception as e:
                        st.error(f"❌ 발송 실패: {e}")
            else:
                st.warning("이메일 주소와 비밀번호를 모두 입력해주세요.")
